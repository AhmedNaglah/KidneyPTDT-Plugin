...
RUN pip install --no-cache-dir --upgrade --ignore-installed pip setuptools && \
    ...
    pip install --no-cache-dir torch==1.10+cu111 -f https://download.pytorch.org/whl/torch_stable.html && \
...
RUN python3 -m slicer_cli_web.cli_list_entrypoint KidneyPTDT --help
